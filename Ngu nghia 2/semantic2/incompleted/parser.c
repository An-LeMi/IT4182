...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      ...
      constantObj = createConstantObject(currentToken->string);
      eat(SB_EQ);

      constValue = compileConstant();      
      constantObj->constAttrs->value = constValue;
      declareObject(constantObj);
...
void compileBlock3(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      ...
      varType = compileType();
      varObject->varAttrs->type = varType;
...
void compileFuncDecl(void) {
  ...
  // Add function to the current Object list
...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_IDENT:
    ...
    // copy const value from declared const
...
ConstantValue* compileConstant2(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    obj = lookupObject(currentToken->string);
    // find ident's value and copy this value
    if(obj == NULL) {
      error(ERR_UNDECLARED_CONSTANT, currentToken->lineNo, currentToken->colNo);
    }
    else if(obj->kind != OBJ_CONSTANT) {
