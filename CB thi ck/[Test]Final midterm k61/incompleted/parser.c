...
Type *compileLValue(void)
{
  ...
  if (var->kind == OBJ_VARIABLE)
  {
    if (var->varAttrs->type->typeClass == TP_ARRAY)
    {
      ...
      varType = duplicateType(var->varAttrs->type); // không chia sẻ giá trị biến
  }
  ...
  {
    printObject(var, currentToken->lineNo);
...
void compileAssignSt(void)
{
  ...
  if (lookAhead->tokenType == SB_COMMA || lookAhead->tokenType == SB_ASSIGN)
  {
    ...
    // printType(t2);
    // printf("%d, %d,", currentToken->lineNo, currentToken->colNo);
    // int count = 0;
    if (count1 < count2)
    {
    ...
    else
    {
      ...
      {
        if (t1List[i]->typeClass == TP_DOUBLE)
        {
          checkNumberType(t2List[i]);
        }
        else
        {
          // printType(t2List[i]);
          // printType(t2List[0]);
          checkTypeEquality(t1List[i], t2List[i]);
        }
      }
    }
  }
  else if (lookAhead->tokenType == SB_CA)
  {
    ...
    if (t1List[0]->typeClass == TP_DOUBLE)
    {
      checkNumberType(rValue1);
      checkNumberType(rValue2);
    }
    else
    {
      checkTypeEquality(rValue1, t1List[0]);
      checkTypeEquality(rValue2, t1List[0]);
    }
    checkTypeEquality(t1List[0], rValue2);
  }
}
